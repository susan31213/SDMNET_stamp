...
function GetOptimizedObjGT(recon_inter_random_dir, type, recon_inter_random, use_struct, use_origin, start_idx, end_idx)
    ...
    if 0 == use_origin && 0 == use_struct
        merge_dir = fullfile(recon_inter_random_dir, 'gt_merge');
    elseif 0 == use_origin && 1 == use_struct
        merge_dir = fullfile(recon_inter_random_dir, 'gt_merge_struct');
    elseif 1 == use_origin && 1 == use_struct
        merge_dir = fullfile(recon_inter_random_dir, 'gt_merge_struct_origin');
    elseif 1 == use_origin && 0 == use_struct
        merge_dir = fullfile(recon_inter_random_dir, 'gt_merge_origin');
    end
    ...
    if 0 == use_struct
        ...
        part_dirs = fullfile(recon_inter_random_dir, cellfun(@(x) ['gt_struc_',x] ,part_names, 'UniformOutput',false));
    end
