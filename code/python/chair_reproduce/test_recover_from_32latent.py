...
with tf.Session(config = model.config) as sess:
    ...
    path = code_folder + '/obj_recon'
    if not os.path.isdir(path):
        os.makedirs(path)

    codefiles = os.listdir(code_folder)
    codefiles.sort()
    codes = []
    for codefile in codefiles:
        if codefile.split('.')[-1] == 'npz':
            code_fullpath = os.path.join(code_folder, codefile)
            code = np.load(code_fullpath)

            codes.append(np.array(code['z']))
            # code['c']
        
        
    print('start inference...')
    model.recover_from_latent(sess, datainfo, codes, path, sdm_only=True)
os.system(f'matlab -nojvm -batch "cd \'/home/susan/SDMNET_stamp/code/matlab\'; GetOptimizedObj2(\'{path}\', \'chair\', 2, 1, 0, 1, {len(codes)})"')