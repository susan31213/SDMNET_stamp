...
with tf.Session(config = model.config) as sess:
    ...
    path = '/home/susan/SDMNET_stamp/code/python/chair_reproduce/copy_paste/new'
    if not os.path.isdir(path):
        os.makedirs(path)

    latent1 = np.array(np.load(args.latent1)['z'])
    latent2 = np.array(np.load(args.latent2)['z'])
        
    print('start inference...')
    model.copy_paste_latent(sess, datainfo, latent1, latent2, path)
